...
class VideoAnimator():
    ...
    def save_video_alternative(self, video, audio, path):
        if not os.path.isabs(path):
            path = os.getcwd() + "/" + path
            audiopath = os.getcwd() + "/example/message_audio.mp3"
        with tempdir() as dirpath:
            ...
            videowriter.close()
            aclip=AudioArrayClip(audio, fps=self.audio_rate)
            #aclip = AudioFileClip(audiopath)
            clip = VideoFileClip(dirpath + '/tmp.avi', audio=False, resize_algorithm="fast_bilinear", target_resolution= (640, 480))
            clip.set_audio(aclip)
            clip.write_videofile(path, codec="libx264", audio_codec="aac")
            
    def save_video_raw(self, video, path="raw.avi"):
        if not os.path.isabs(path):
            path = os.getcwd() + "/" + path
        videowriter = imageio.get_writer(path, fps=self.video_rate)
        for i in range(video.shape[0]):
            frame = np.rollaxis(video[i, :, :, :], 0, 3)
            videowriter.append_data(frame)
        videowriter.close()
        clip = VideoFileClip(path, audio=False)
        clip.write_videofile('input.avi', codec='rawvideo')
